  ...
  df <-
    ...
    dplyr::summarize(
      ...
      Pct_PEPFAR_Facilities_WithEMR =
        length(
          .data$facilityuid[.data$emr_present == TRUE &
                              !is.na(.data$emr_present) &
                              .data$reported_by %in% c("PEPFAR", "Both")]) /
        length(
          .data$facilityuid[.data$reported_by %in% c("PEPFAR", "Both")]),
      Pct_PEPFAR_Facilities_ReportedBy_Both_WithEMR =
        ...
        length(
          .data$facilityuid[.data$reported_by == "Both"]),
      MOH_Results_Total = sum(
        .data$moh[.data$reported_by %in% c("Both", "MOH")], na.rm = TRUE),
      PEPFAR_Results_Total = sum(
        .data$pepfar[.data$reported_by %in% c("Both", "PEPFAR")], na.rm = TRUE),
      MOH_Results_FacilitiesReportedByBoth = sum(
        .data$moh[.data$reported_by == "Both"], na.rm = TRUE),
      PEPFAR_Results_FacilitiesReportedByBoth = sum(
        .data$pepfar[.data$reported_by == "Both"], na.rm = TRUE),
      PEPFAR_Results_FacilitiesReportedBy_PEPFAROnly = sum(
        .data$pepfar[.data$reported_by == "PEPFAR"], na.rm = TRUE),
      Concordance = sum(
        .data$level3_concordance[.data$reported_by == "Both"], na.rm = TRUE),
      ConcordanceOfFacilitiesWithEMR = sum(
        .data$emr_concordance[.data$reported_by == "Both" &
                                .data$emr_present == TRUE &
                                !is.na(.data$emr_present)], na.rm = TRUE),
      ConcordanceOfFacilitiesWithOutEMR = sum(
        .data$emr_concordance[.data$reported_by == "Both" &
    ...
    dplyr::mutate(
      across(
        c(.data$MOH_Results_FacilitiesReportedByBoth,
    ...
    dplyr::mutate(
      across(
        c(.data$Pct_PEPFAR_Reported_Facilities_ReportedBy_MOH,
